...
export class TravelItineraryAI {
  private async getAIModel() {
    ...
    const travelSettings = aiSettings?.travelItinerary
    
    // Determine which provider and model to use
    const provider = travelSettings?.provider === 'inherit' || !travelSettings?.provider 
      ? aiSettings?.defaultProvider || 'openai'
      : travelSettings.provider
    
    const temperature = travelSettings?.temperature || 0.7
    const maxTokens = travelSettings?.maxTokens || 4000
    ...
    switch (provider) {
      case 'anthropic':
        modelName = travelSettings?.model || 'claude-3-sonnet-20240229'
        model = anthropic(modelName)
        break
      case 'google':
        modelName = travelSettings?.model || 'gemini-pro'
        model = google(modelName)
        break
      case 'openai':
      default:
        modelName = travelSettings?.model || 'gpt-4-turbo-preview'
  ...
  async generateItinerary(request: ItineraryRequest): Promise<GeneratedItinerary> {
    ...
    try {
      const { model, temperature, maxTokens } = await this.getAIModel()
      
      const { text } = await generateText({
        model,
        prompt: `You are a professional travel planner creating detailed itineraries. ${prompt}`,
        temperature,
        maxTokens,
      })
